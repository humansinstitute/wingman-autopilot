...
describe('home session groups', () => {
  const sessions = [
    ...
    {
      id: 'orchestration-1',
      metadata: { AGENT: true, billingMode: 'subscription', role: 'agent-work', bindingType: 'flow_orchestration' },
      origin: { type: 'agent-work', id: 'run-1' },
    },
    {
      id: 'chat-1',
  ...
  test('classifies home session groups by origin and metadata', () => {
    ...
    expect(getHomeSessionGroup(sessions[2])).toBe('task-dispatch');
    expect(getHomeSessionGroup(sessions[3])).toBe('chat-dispatch');
    expect(getHomeSessionGroup(sessions[4])).toBe('agent-sessions');
    expect(getHomeSessionGroup(sessions[5])).toBe('my');
  });
  ...
  test('filters sessions for the selected home group', () => {
    ...
    expect(filterSessionsForHomeGroup(sessions, 'task-dispatch').map((session) => session.id)).toEqual(['task-1', 'orchestration-1']);
