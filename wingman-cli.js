  ...
  try {
    ...
    tmux.on('exit', async () => {
      try {
        ...
        const sessionExists = remainingSessions.includes(sessionName);

        if (sessionExists) {
          console.log(`${colors.green}Detached from session: ${sessionName}${colors.reset}`);
...
async function createNewSession() {
  while (true) {
    ...
    const sessionNameInput = await question("Enter session name (or 'q' to cancel): ");
    ...
    try {
      ...
      const connectChoice = await question('Session exists. Connect instead? (y/n): ');
      if (connectChoice.toLowerCase() === 'y') {
        await connectToSpecificSession(sessionName);
        return;
      }
      continue;
    } catch (error) {
      // Session does not exist, continue with creation
    }
    ...
    try {
      ...
      console.log(`${colors.red}Failed to create session: ${error.message}${colors.reset}`);
      await sleep(1500);
    }
  }
}
